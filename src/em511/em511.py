...
class Em511:
    ...
    EM511_REGISTER_A_DMD = 0x003A
    EM511_REGISTER_A_DMD_PEAK = 0x003C
    EM511_REGISTER_W = 0x0004
    EM511_REGISTER_W_DMD = 0x000A
    EM511_REGISTER_W_DMD_PEAK = 0x000C
    EM511_REGISTER_HZ = 0x000F
    EM511_REGISTER_PASSWORD = 0x1000
    EM511_REGISTER_KWH_TOT = 0x0010
    EM511_REGISTER_KWH_PARTIAL = 0x0014
    EM511_REGISTER_HOUR_COUNTER = 0x002C
    EM511_REGISTER_LIFETIME_COUNTER = 0x0030
    EM511_REGISTER_HOUR_COUNTER_PART = 0x0036
    EM511_REGISTER_DMD_INTEGRATION_TIME = 0x1010
    EM511_REGISTER_ALARM_MODE = 0x1015
    ...
    @alarm_mode.setter
    def alarm_mode(self, value: int) -> None:
        """Alarm mode.

        Value:
        1=Active Power [kW], 2=Apparent Power [kVA],
        3=Reactive Power [kVAR], 4=Power Factor,
        5=Currents [A], 6=Voltages [V] (default=1)

        Args:
            value (int): Set alarm mode.

        Raises:
            ModbusException: If failed to write to single register.
            ValueError: If alarm mode  value is out of range.
        """
        if not (self.ALARM_MODE_MIN_VALUE <= value <= self.ALARM_MODE_MAX_VALUE):
            msg = f"Invalid alarm mode value: {value}. Must be between 1 and 6."
            raise ValueError(msg)
